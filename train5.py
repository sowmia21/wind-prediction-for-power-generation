# train5.py - Ridge Regression model for wind power prediction
...
def train(csv_path, test_ratio=0.2):
    ...
    joblib.dump(model, os.path.join("artifacts", "ridge_model.pkl"))
    joblib.dump(scaler, os.path.join("artifacts", "ridge_scaler.pkl"))
    with open(os.path.join("artifacts", "ridge_features.json"), "w") as f:
        json.dump(feat_names, f, indent=2)
    with open(os.path.join("artifacts", "ridge_metrics.json"), "w") as f:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Ridge Regression on wind dataset")
