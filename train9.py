# train9.py - Gradient Boosting Regressor
import argparse
import os
import json
import pandas as pd
...
def train(csv_path, test_ratio=0.2):
    ...
    joblib.dump(model, os.path.join("artifacts", "model_gbr.pkl"))
    joblib.dump(scaler, os.path.join("artifacts", "scaler_gbr.pkl"))
    with open(os.path.join("artifacts", "features_gbr.json"), "w") as f:
        json.dump(feat_names, f, indent=2)
    with open(os.path.join("artifacts", "metrics_gbr.json"), "w") as f:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Gradient Boosting on wind dataset")
