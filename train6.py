# train6.py - Lasso Regression model for wind power prediction
...
def train(csv_path, test_ratio=0.2):
    ...
    joblib.dump(model, os.path.join("artifacts", "lasso_model.pkl"))
    joblib.dump(scaler, os.path.join("artifacts", "lasso_scaler.pkl"))
    with open(os.path.join("artifacts", "lasso_features.json"), "w") as f:
        json.dump(feat_names, f, indent=2)
    with open(os.path.join("artifacts", "lasso_metrics.json"), "w") as f:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Lasso Regression on wind dataset")
