...
# Rated power of turbine (used for relative error display)
...
def train(csv_path, test_ratio=0.2):
    ...
    if y is None:
        raise RuntimeError("Target 'Power' not found after mapping.")
    ...
    print(f"R² score: {r2:.2f} (variance explained)")
    print(f"MAE relative to rated power: {mae/RATED_POWER_KW*100:.2f}%")
    print(f"RMSE relative to rated power: {rmse/RATED_POWER_KW*100:.2f}%")
    print("----------------------------------\n")

    # Show sample predictions
    print("Sample Predictions vs Actual Power:")
    for i in range(min(5, len(y_test))):
        print(f"Predicted: {preds[i]:.2f} kW | Actual: {y_test.iloc[i]:.2f} kW")

    return metrics

# ---------------- COMMAND LINE ----------------
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train linear regression on wind dataset")
